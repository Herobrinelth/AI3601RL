...
class CQLCDSAgent:
    ...
    def conservative_data_share(self, batch_main, batch_share):
        ...
        obs_m = to_tensor(batch_main['observations'], self.device)
        action_m = to_tensor(batch_main['actions'], self.device)
        reward_m = to_tensor(batch_main['rewards'], self.device).squeeze(-1)
        discount_m = to_tensor(batch_main['discounts'], self.device).squeeze(-1)
        next_obs_m = to_tensor(batch_main['next_observations'], self.device)
        #print("main samples:", obs_m.shape, action_m.shape, reward_m.shape, discount_m.shape, next_obs_m.shape)
        ...
        obs_s = to_tensor(batch_share['observations'], self.device)
        action_s = to_tensor(batch_share['actions'], self.device)
        reward_s = to_tensor(batch_share['rewards'], self.device).squeeze(-1)
        discount_s = to_tensor(batch_share['discounts'], self.device).squeeze(-1)
        next_obs_s = to_tensor(batch_share['next_observations'], self.device)
        #print("obs:", obs_s.shape, action_s.shape, reward_s.shape, discount_s.shape, next_obs_s.shape)
        # calculate the conservative Q value
        with torch.no_grad():
            conservative_q_value, _ = self.critic(obs_s, action_s)
        conservative_q_value = conservative_q_value.squeeze().cpu().numpy()
        ...
        # extract the samples
        obs_all.append(obs_s[top_index])
        action_all.append(action_s[top_index])
        next_obs_all.append(next_obs_s[top_index])
        reward_all.append(reward_s[top_index])
        discount_all.append(discount_s[top_index])
    ...
    def update(self, batch_main, batch_share, step, total_step):
        metrics = dict()
